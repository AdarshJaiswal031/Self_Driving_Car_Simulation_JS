...
networkCanvas.width = 290;
const carCtx = carCanvas.getContext("2d");
const networkCtx = networkCanvas.getContext("2d");
const road = new Road(carCanvas.width / 2,
    carCanvas.width * 0.9);
...
for (let i = 0; i < 10; i++) {
    traffic.push(new Car(road.getLaneCenter(Math.abs(Math.random() * 3)), -2600 - (i * 100), 30, 50, "DUMMY", 2))
...
if (localStorage.getItem("bestBrain")) {
    for (let i = 0; i < cars.length; i++) {
        ...
        if (i != 0) {
            NeuralNetworks.mutate(cars[i].brain, 0.5)
